...
pub fn generate(out_dir: &Path) -> CarbideCliResult<()> {
    ...
    for sub in root.get_subcommands() {
        ...
        render_node(sub, vec![BIN.to_string(), name.clone()], domain, &man_dir, &dir)?;
...
fn breadcrumb(path: &[String], domain: CliDomain) -> String {
    let mut parts =
        vec![format!("[{} commands](../../{}.md)", domain.title(), slug(domain))];
    ...
    parts.push(format!(
        "**{}**",
        path.last().expect("a command path always has at least the binary")
    ));
...
fn strip_sections(md: &str, names: &[&str]) -> String {
    ...
    for line in md.lines() {
        if let Some(heading) = line.strip_prefix("## ") {
            skipping = names
                .iter()
                .any(|n| heading.trim().eq_ignore_ascii_case(n));
            if skipping {
                continue;
            }
        }
