...
| `nico` | `ComponentPowerControl`, `GetComponentInventory`, `UpdateComponentFirmware`, `GetComponentFirmwareStatus` | New Component Manager dispatch path, identical to `nvswitch/nico` and `powershelf/nico`. Honours `info.SubTargets` (BMC, BIOS, ...) and forwards `target_version` verbatim to Core (the SoT firmware-object identifier). Does **not** consume the firmware time window — Core dispatches immediately. For power control, rack-associated RMS machines use the rack state controller unless the request explicitly bypasses it, while standalone machines retain Core's Redfish path. `compute_tray_use_state_controller=true` controls rack compute-tray firmware routing (see `crates/component-manager/src/config.rs`). |
