...
Both EXTERNAL-IP and TYPE=`LoadBalancer` must be populated. A `<pending>` IP indicates a MetalLB issue — see [Reference Installation — MetalLB troubleshooting](../getting-started/installation-options/reference-install.md#metallb-loadbalancer-services-stuck-in-pending).
...
- [Reference Installation](../getting-started/installation-options/reference-install.md) — phase-by-phase install with troubleshooting for MetalLB, DNS, and `nico-api`.
